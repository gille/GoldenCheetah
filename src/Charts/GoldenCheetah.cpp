...
GcChartWindow::chartPropertiesFromString(QString contents) {
    ...
    if (parseError.error == QJsonParseError::NoError && json.isObject()) {
        ...
        if (root.contains("CHART") && root["CHART"].isObject()) {
            
            QJsonObject c = root["CHART"].toObject();
            QString type, view;
            QMap<QString, QString> m;

            // top level - type and view
            if (c.contains("TYPE")) type = c["TYPE"].toString();
            if (c.contains("VIEW")) view = c["VIEW"].toString();
            ...
            if (c.contains("PROPERTIES") && c["PROPERTIES"].isObject()) {
                QJsonObject p = c["PROPERTIES"].toObject();
