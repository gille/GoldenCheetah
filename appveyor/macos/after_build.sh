...
# Use rsync with fault-tolerance for broken symlinks in cached site-packages
if [ -d "../site-packages" ]; then
    rsync -a --ignore-missing-args ../site-packages/ "$SITE_PACKAGES/" 2>/dev/null || cp -RL ../site-packages "$SITE_PACKAGES" 2>/dev/null || {
        echo "Warning: site-packages copy had errors - some Python packages may be missing"
        # Fallback: create empty directory and continue
        mkdir -p "$SITE_PACKAGES"
    }
else
    echo "Warning: site-packages not found - Python packages may be missing"
    mkdir -p "$SITE_PACKAGES"
fi
