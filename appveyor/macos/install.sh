...
# Python mandatory packages - refresh cache if folder is empty or key packages are missing
if [ -z "$(ls -A site-packages 2>/dev/null)" ] || [ ! -d "site-packages/numpy" ]; then
    echo "Installing/refreshing Python packages in site-packages..."
    rm -rf site-packages
    mkdir -p site-packages
    python3 -m pip install -q -r src/Python/requirements.txt -t site-packages
fi
